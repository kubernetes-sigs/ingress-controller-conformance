...
func init() {
	...
	hostRulesCheck.AddCheck(hostRulesExactMatchSecureCheck)
...
var hostRulesExactMatchSecureCheck = &checks.Check{
	Name:        "host-rules-exact-match-secure",
	Description: "Ingress with exact host rule should terminate TLS and send traffic to the correct backend service (foo.bar.com matches foo.bar.com)",
	APIVersions: apiversion.All,
	RunRequest: &checks.Request{
		IngressName: "host-rules",
		Path:        "",
